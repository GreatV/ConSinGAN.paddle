...
def get_scale_factor(opt):
    ...
    num_scales = math.ceil(math.log(math.pow(opt.min_size / min(real.shape[
        2], real.shape[3]), 1), opt.scale_factor_init)) + 1
    ...
    if opt.num_training_scales > 0:
        while num_scales > opt.num_training_scales:
            ...
            num_scales = math.ceil(math.log(math.pow(opt.min_size / min(
                real.shape[2], real.shape[3]), 1), opt.scale_factor_init)) + 1
...
if __name__ == '__main__':
    ...
    if (opt.train_mode == 'generation' or opt.train_mode == 'retarget' or 
        opt.train_mode == 'animation'):
    ...
    copytree(osp.join(current_path, 'ConSinGAN'), osp.join(dir2save,
        'ConSinGAN'))
