...
def imresize_in(im, scale_factor=None, output_shape=None, kernel=None,
    ...
    if type(kernel) == np.ndarray and scale_factor[0] <= 1:
        return numeric_kernel(im, kernel, scale_factor, output_shape,
            kernel_shift_flag)
    method, kernel_width = {'cubic': (cubic, 4.0), 'lanczos2': (lanczos2, 
        4.0), 'lanczos3': (lanczos3, 6.0), 'box': (box, 1.0), 'linear': (
        linear, 2.0), None: (cubic, 4.0)}.get(kernel)
    ...
    for dim in sorted_dims:
        if scale_factor[dim] == 1.0:
            continue
        weights, field_of_view = contributions(im.shape[dim], output_shape[
            dim], scale_factor[dim], method, kernel_width, antialiasing)
...
def fix_scale_and_size(input_shape, output_shape, scale_factor):
    ...
    if output_shape is not None:
        output_shape = list(np.uint(np.array(output_shape))) + list(input_shape
            [len(output_shape):])
...
def contributions(in_length, out_length, scale, kernel, kernel_width,
    ...
    field_of_view = np.squeeze(np.uint(np.expand_dims(left_boundary, axis=1
...
def resize_along_dim(im, dim, weights, field_of_view):
    ...
    weights = np.reshape(weights.T, list(weights.T.shape) + (np.ndim(im) - 
        1) * [1])
...
def numeric_kernel(im, kernel, scale_factor, output_shape, kernel_shift_flag):
    ...
    return out_im[np.round(np.linspace(0, im.shape[0] - 1 / scale_factor[0],
        output_shape[0])).astype(int)[:, None], np.round(np.linspace(0, im.
        shape[1] - 1 / scale_factor[1], output_shape[1])).astype(int), :]


def kernel_shift(kernel, sf):
    current_center_of_mass = measurements.center_of_mass(kernel)
    wanted_center_of_mass = np.array(kernel.shape) / 2 + 0.5 * (sf - kernel
        .shape[0] % 2)
