...
def generate_samples(netG, reals_shapes, noise_amp, scale_w=1.0, scale_h=
    1.0, reconstruct=False, n=50):
    if reconstruct:
    ...
    if scale_w == 1.0 and scale_h == 1.0:
        ...
        reals_shapes = [[r_shape[0], r_shape[1], int(r_shape[2] * scale_h),
            int(r_shape[3] * scale_w)] for r_shape in reals_shapes]
        dir2save_parent = os.path.join(dir2save,
    ...
    for idx in range(n):
        ...
        functions.save_image('{}/gen_sample_{}.jpg'.format(dir2save_parent,
            idx), sample.detach())
...
if __name__ == '__main__':
    ...
    if paddle.device.cuda.device_count() >= 1:
        paddle.device.set_device(device=opt.gpu)
        opt.place = 'cuda:{}'.format(opt.gpu)
    dir2save = os.path.join(opt.model_dir, 'Evaluation')
    make_dir(dir2save)
    print('Loading models...')
    netG = paddle.load(path='%s/G.pth' % opt.model_dir)
    fixed_noise = paddle.load(path='%s/fixed_noise.pth' % opt.model_dir)
    reals = paddle.load(path='%s/reals.pth' % opt.model_dir)
    ...
    if opt.train_mode == 'generation' or opt.train_mode == 'retarget':
        ...
        with paddle.no_grad():
            ...
            rs0 = generate_samples(netG, reals_shapes, noise_amp, n=opt.
                num_samples)
            generate_samples(netG, reals_shapes, noise_amp, scale_w=2,
                scale_h=1, n=opt.num_samples)
            generate_samples(netG, reals_shapes, noise_amp, scale_w=1,
                scale_h=2, n=opt.num_samples)
            generate_samples(netG, reals_shapes, noise_amp, scale_w=2,
                scale_h=2, n=opt.num_samples)
    elif opt.train_mode == 'harmonization' or opt.train_mode == 'editing':
        ...
        if opt.train_mode == 'editing':
            fixed_noise[0] = fixed_noise[0
                ] + opt.noise_scaling * functions.generate_noise([opt.nc_im,
                fixed_noise[0].shape[2], fixed_noise[0].shape[3]], device=
                opt.place)
        ...
        for _start_scale in range(3):
            for _beta in range(80, 100, 5):
                functions.generate_gif(dir2save, netG, fixed_noise, reals,
                    noise_amp, opt, alpha=0.1, beta=_beta / 100.0,
                    start_scale=_start_scale, num_images=100, fps=10)
